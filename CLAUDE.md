...
You MUST follow these 6 steps in order. **Never write implementation code without a spec and a plan.**
...
- Do not skip or merge steps. Each artifact (`spec.md`, `plan.md`, `task.md`, `evidence.md`) must exist before moving to the next step.
- Keep each source file focused on a single responsibility.
- Use `@google/genai` only inside the Image Generator module — do not scatter API calls.
- NEVER modify `.feature` files during implementation — only during Specify or Iterate phases.
- Use the SDD skills in `.claude/skills/` for guidance on each phase.
